...
async function fetchGoalOverview(): Promise<GoalOverview> {
    ...
    return {
        ...
        emergencyFund: data.emergencyFund || undefined,
    };
}
    ...
    return (
        <Card className={`overflow-hidden border shadow-xl shadow-black/20 backdrop-blur ${
            ...
            <div className="px-4 py-5 sm:px-6 sm:py-6 lg:px-8">
                ...
                {!ef.isComplete && (
                    <div className="mt-4 flex items-start gap-3 rounded-xl border border-orange-500/20 bg-orange-950/20 px-4 py-3">
                        ...
                        <div className="text-xs text-orange-200/80 leading-relaxed">
                            <span className="font-semibold text-orange-300">Goal allocations are paused.</span>{" "}
                            Every rupee of monthly savings capacity is being directed to the emergency fund first.
                            Other goals will start receiving allocations once the emergency fund is fully funded.
                            {ef.monthsToComplete && (
                                <span> At your current savings rate, this will happen in approximately{" "}
                                    <span className="font-semibold text-orange-200">{ef.monthsToComplete} month{ef.monthsToComplete === 1 ? "" : "s"}</span>.
                                </span>
                            )}
                        </div>
                    </div>
                ...
                {ef.isComplete && (
                    <div className="mt-4 flex items-start gap-3 rounded-xl border border-emerald-500/20 bg-emerald-950/20 px-4 py-3">
                        <span className="mt-0.5 text-emerald-400 text-base">✅</span>
                        <div className="text-xs text-emerald-200/80 leading-relaxed">
                            <span className="font-semibold text-emerald-300">Emergency fund is fully funded!</span>{" "}
                            Your financial safety net covers {ef.targetMonths} months of expenses.
                            Monthly capacity is now available for all your other goals.
                        </div>
                    </div>
                )}
            </div>
