...
function MetricBlock({ label, value, tone, note }: { label: string; value: string | number; tone?: string; note?: string }) {
    ...
    return (
        <div className={cn("rounded-xl border p-3", toneStyles(tone))}>
            ...
            <div className="mt-2 text-base font-semibold text-foreground">{displayValue}</div>
        ...
        <ArtifactShell title={title} icon={<ShieldAlert className="size-4 text-amber-300" />}>
            <div className="space-y-3">
                {safeItems.map((item, i) => (
                    <div key={`${item.title}-${i}`} className={cn("rounded-xl border p-3", toneStyles(item.severity || "warning"))}>
                        <div className="flex items-start gap-3">
                ...
                {safeItems.map((item, index) => (
                    <div key={`${item.label}-${index}`} className="flex gap-3">
                        <div className="flex flex-col items-center">
                            ...
                            {index < safeItems.length - 1 ? <div className="mt-2 h-full w-px flex-1 bg-border/60" /> : null}
                        </div>
                ...
                <table className="min-w-full border-separate border-spacing-y-2">
                    <thead>
                        <tr>
                            ...
                            {safeCols.map((column, i) => (
                                <th key={`col-${i}`} className="px-3 py-2 text-left text-[10px] font-semibold uppercase tracking-[0.22em] text-muted-foreground">
                                    {column}
                                </th>
                            ))}
                        </tr>
                    ...
                    <tbody>
                        {safeRows.map((row, rowIdx) => {
                            ...
                            return (
                                <tr key={`row-${rowIdx}`} className="rounded-xl border border-border/70 bg-muted/30">
                                    <td className="rounded-l-xl px-3 py-3 text-sm font-semibold text-foreground">{row.label || "—"}</td>
                                    {paddedValues.map((value, colIdx) => (
                                        <td
                                            key={`${rowIdx}-${colIdx}`}
                                            className={cn("px-3 py-3 text-sm text-muted-foreground", colIdx === paddedValues.length - 1 ? "rounded-r-xl" : "")}
                                        >
                                            {value}
                                        </td>
                                    ))}
